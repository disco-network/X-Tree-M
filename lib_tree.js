...
// Class 'lib_tree'
export function lib_tree(
  gui_headline_context,
  lang_headline,
  gui_tree_context,
  current_usecase,
  current_panel,
  ...
  const handler = (operation, id, event) =>
    this.dispatcher(
      this.current_usecase,
      this.current_panel,
      operation,
      id,
      c_KEYB_MODE_NONE,
...
function lib_tree_print_item_rec(
  pos,
  tree_diff,
  selected_positions,
  is_gui_id_expanded,
  ...
  const selected =
    selected_positions.map((pos) => pos.get_gui_id()).indexOf(gui_id) >= 0;
  const expanded = is_gui_id_expanded(gui_id);
  const on_click = function (event) {
    return self.handler("tree_select", gui_id + "_a", event);
  };
  ...
  if (create_child) {
    ...
    const new_vnode = this.print_item(
      "N0",
      new_node,
      () => {},
      false,
  ...
  const diff_new_children = tree_diff.get_new_children(pos);
  const diff_new_name = tree_diff.get_new_name(pos);
  return this.print_item(
    pos.get_gui_id(),
    pos.get_node(),
    on_click,
    selected,
...
function lib_tree_print_item(
  ...
  const new_children_vnode =
    diff_new_children.length > 0
      ? h("span.diff-new-children", [`(${diff_new_children.length} new)`])
      : undefined;

  const new_name_vnode =
    diff_new_name !== null
      ? h("div.diff-new-name", [diff_new_name])
      : undefined;

  const name_class = node.is_deleted === 1 ? "div deleted" : "div";
  const name_vnode = h(
    "div#" + node_gui_id + "_div",
    {
      props: {
  ...
  return h(
    "li#" + node_gui_id + "_li",
    { key: node_gui_id, props: { className: ret_class } },
    [name_vnode, ul_vnode]
  );
}
...
function get_disptype_tree_vnode(state) {
  ...
  const is_gui_id_expanded = (gui_id) =>
    state.expanded.has(tree.locate(gui_id).get_downward_path());
  ...
  for (
    var predecessor = tree.locate_pivot();
    predecessor.locate_parent() !== null;
    predecessor = predecessor.locate_parent()
  ) {
    // prepare variables
    ...
    const visible_name =
      position.locate_parent() === null ? "[" + node.name + "]" : node.name;
    const on_click = function (event) {
      return self.handler("explorer_select", gui_id_a, event);
    };
    const on_click_multi = function (event) {
      return self.handler(
        "open_parent_menu",
        predecessor.get_gui_id() + "_a",
        event
      );
    };
...
function lib_tree_print_tree(state) {
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  var retval;
  if (global_setup.display_type == 0) retval = this.print_disptype_tree(state);
  else
    retval = this.print_disptype_bubbles(
      tree_obj,
      sel_elem_id,
      selected_item_in_tree
    );

  const end_time = new Date();
  console.log(
    "Tree printing took " + (end_time - start_time) + " milliseconds."
  );
...
function lib_tree_get_type_no(typeInternalStr) {
  for (var i = 0; i < c_LANG_LIB_TREE_ELEMTYPE.length; i++) {
    if (typeInternalStr == c_LANG_LIB_TREE_ELEMTYPE[i][0]) return i - 1;
  }
  return -1;
}
...
function lib_tree_print_multi_parent_menu(parent_list) {
  ...
  var html_text =
    "&nbsp;<I>" +
    c_LANG_LIB_TREE_MSG_MULTI_CHOICE[global_setup.curr_lang] +
    " :</I><BR><BR>";
  var gui_context = document.getElementById(this.gui_tree_context);
  for (var a = 0; a < parent_list.length; a++) {
    const on_click = function (event) {
      return self.handler("parent_menu_select", this.id, event);
    };
    click_handlers.push(on_click);
    html_text =
      html_text +
      '&nbsp;<a id="' +
      parent_list[a].elem_id +
      '_a">' +
      parent_list[a].name +
      "</a><br>";
  }
  ...
  for (var i = this.curr_tree_obj.tree_nodes.length - 1; i >= 0; i--) {
    ...
    if (this.curr_tree_obj.tree_nodes[i].gui_id == srch_gui_id) {
      if (this.curr_tree_obj.tree_nodes[i].isMultiPar) {
        ...
        retval[retval_idx].parent_id = this.curr_tree_obj.tree_nodes[
          i
        ].parent_elem_id;
        retval_idx++;
      }
  ...
  for (var i = 0; i < cut_items.length; i++) {
    ...
    if (my_gui_id.length > 0) {
      for (var j = 0; j < my_gui_id.length; j++) {
        if (
          this.get_item_data(my_gui_id[j]).parent_elem_id[0] ==
          my_parent_elem_id[0]
        ) {
          if (is_marked)
            if (this.get_item_data(my_gui_id[j]).subtree % 2 == 1)
              document.getElementById(my_gui_id[j] + "_a").style.color =
                "#FFB0B0";
            else
              document.getElementById(my_gui_id[j] + "_a").style.color =
                "#D0D0D0";
...
function print_disptype_bubbles(tree_obj, sel_elem_id, selected_item_in_tree) {
  ...
  // constants
  var c_PARENT_FILL = "#D0D0FF";
  var c_PARENT_STROKE = "#3030FF";
  var c_TARGET_FOR_PARENT = 1;
  var c_PARENT_LINK_COLOR = "#00C";
  var c_SIBLING_FILL = "#80FF80";
  var c_SIBLING_STROKE = "#008000";
  var c_TARGET_FOR_SIBLING = 2;
  var c_SIBLING_LINK_COLOR = "#0C0";
  var c_CHILD_FILL = "#FF60FF";
  var c_CHILD_STROKE = "#8F008F";
  var c_TARGET_FOR_CHILD = 3;
  var c_CHILD_LINK_COLOR = "#C00";
  ...
  var graph = {
    nodes: [
      // Element
      {
        id: selected_item_in_tree.gui_id + "_a",
        name: selected_item_in_tree.name.replace(/&rsaquo;/g, ">"),
        fill: "#FFFFB0",
        stroke: "#0000C0",
        symbol: "",
      },
  ...
  if (curr_node != undefined) {
    ...
    else graph.nodes[curr_node_idx].symbol = "symbol_unknown.gif";
    graph.nodes[curr_node_idx].fill = c_PARENT_FILL;
    graph.nodes[curr_node_idx].stroke = c_PARENT_STROKE;
  ...
  if (tree_obj.tree_nodes != undefined) {
    for (var i = 0; i < tree_obj.tree_nodes.length; i++) {
      ...
      // Sibling Nodes
      if (
        curr_node.parent_gui_id == selected_item_in_tree.parent_gui_id &&
        curr_node.elem_id != selected_item_in_tree.elem_id
      ) {
        graph.nodes[curr_node_idx] = {};
        graph.nodes[curr_node_idx].id = curr_node.gui_id + "_a";
        graph.nodes[curr_node_idx].name = curr_node.name.replace(
          /&rsaquo;/g,
          ">"
        );
        if (curr_node.type != "none")
        ...
        graph.nodes[curr_node_idx].fill = c_SIBLING_FILL;
        graph.nodes[curr_node_idx].stroke = c_SIBLING_STROKE;
        graph.links[curr_link_idx] = {};
        graph.links[curr_link_idx].source = curr_node_idx++;
      ...
      if (curr_node.parent_gui_id == selected_item_in_tree.gui_id) {
        ...
        graph.nodes[curr_node_idx].fill = c_CHILD_FILL;
        graph.nodes[curr_node_idx].stroke = c_CHILD_STROKE;
        graph.links[curr_link_idx] = {};
        graph.links[curr_link_idx].source = curr_node_idx++;
  ...
  on_click = function (event) {
    return self.handler("explorer_select", this.id, event);
  };
  ...
  var drawGraph = function (graph) {
    ...
    var node = gnodes
      ...
      .attr("width", function (d) {
        return elementWidth * d.name.length;
      })
    ...
    var labels = gnodes
      ...
      .style("font", function (d) {
        if (d.index == 0) return "10pt courier";
        else return "10pt courier";
      })
      .call(force.drag);
    ...
    force.on("tick", function () {
      ...
      gnodes.attr("transform", function (d) {
        return (
          "translate(" +
          [d.x - (elementWidth * d.name.length) / 2, d.y - elementHeight / 2] +
          ")"
        );
      });
    });
